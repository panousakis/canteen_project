...
$sql = "SELECT id, customer_name, total, order_date FROM orders ORDER BY $order";
...
<div class="card">
    <h2>Παραγγελίες</h2>
    <p>Από αυτή τη σελίδα μπορείτε να προσθέσετε παραγγελίες και να δείτε τη λίστα τους.</p>
        ...
        <?php if ($result && $result->num_rows > 0): ?>
            <?php while ($row = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <td><?= !empty($row['order_date']) ? date("d/m/Y H:i", strtotime($row['order_date'])) : '-'; ?></td>
                </tr>
            <?php endwhile; ?>
        <?php else: ?>
            <tr>
